...
main.b <- ggplot(data = b_series) +
  ...
  geom_ribbon(aes(x = DateTime_local, y = bathy, ymin = 200, ymax = 1000), color = "grey33", alpha = 0.4) +
  ylab("Depth (m)") +
  xlab("Month") +
...
blue.bin.depth <- blue.bin.depth %>% filter(tot >= 190)
...
sub.b <- ggplot() +
  ...
  scale_y_continuous(limits = c(-60,60), breaks = c(-60,-50,-40,-30,-20,-10, 0,10,20,30,40,50,60), labels = c(60,50,40,30,20,10,0,10,20,30,40,50,60), position = "right") +
  xlab("Depth (m)") +
  ylab("Time at Depth (%)") +
  coord_flip() +
  theme_classic() 
...
mako.bin.depth <- mako.bin.depth %>% filter(tot >= 190)
...
ggplot(data = r_df) +
  ...
  geom_contour(aes(x = x, y = y, z = Elevation.relative.to.sea.level), color = "white", breaks = c(-200, -1000)) +
  scale_fill_gradient(low = "black", high = "grey") +
  scale_x_continuous(minor_breaks = c(-70, -50, -30, -10)) +
  scale_y_continuous(minor_breaks = c(15, 25, 35, 45, 55)) +
  coord_quickmap() +
...
low <- data.frame(
  ...
  lunar = as.factor(rep(rep(c(1,2), each = 81), times = 17)),
  n2 = rep(c(1.491943e-05), times = 2754),
...
high <- data.frame(
  ssh = rep(seq(-1,1, 0.025), times = 34), 
  lunar = as.factor(rep(rep(c(1,2), each = 81), times = 17)),
  n2 = rep(c(7.460897e-05), times = 2754),
  ssh_sd = rep(c(0.02605838), times = 2754),
...
op1.1 <- lssh %>% filter(lunar == 1 & n2 == 1.491943e-05)
op1.2 <- lssh %>% filter(lunar == 1 & n2 == 7.460897e-05)
op2.1 <- lssh %>% filter(lunar == 2 & n2 == 1.491943e-05)
op2.2 <- lssh %>% filter(lunar == 2 & n2 == 7.460897e-05)
...
ggplot(data = filter(lssh, species == "I.oxyrinchus")) + # plot relationships for makos
  geom_line(data = filter(opall, species == "I.oxyrinchus"), aes(x = ssh, y = value, color = variable), alpha = 1.2) +
...
ggplot(data = filter(lssh, species == "P.glauca")) + # plot relationships for blues
  geom_line(data = filter(opall, species == "P.glauca"), aes(x = ssh, y = value, color = variable), alpha = 1.2) +
  geom_path(aes(x = ssh, y = value, color = variable), alpha = 0.15) +
  facet_grid(lunar~n2) + 
  labs(color = "Cluster") +
  ylab("Probability") +
...
p_lunar <- data.frame(
  ...
  lunar = as.factor(rep(rep(c(1,2), each = 1), times = 17)),
